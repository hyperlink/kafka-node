...
HighLevelConsumer.prototype.fetch = function () {
  ...
  try {
    ...
    this.emit('error', new Error(util.format('HLC fetch() failed with error: %j payloads: %j', e, this.topicPayloads)));
  }
};
...
HighLevelConsumer.prototype.offsetRequest = function (payloads, cb) {
  try {
    ...
    this.emit('error', new Error(util.format('HLC offsetRequest() failed with error: %j payloads: %j', e, payloads)));
    cb(e);
  }
};
